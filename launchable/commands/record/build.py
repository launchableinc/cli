...
def build(ctx, build_number, source, with_commit):
    ...
    sources = [(name, subprocess.check_output("git rev-parse HEAD".split(),
                                              cwd=repo_dist).decode()) for name, repo_dist in repos]
    submodules = []
    ...
    try:
        ...
        url = "https://api.mercury.launchableinc.com/intake/organizations/{}/workspaces/{}/builds".format(
            org, workspace)
