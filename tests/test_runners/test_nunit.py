...
class NUnitTest(CliTestCase):
    ...
    def test_subset(self):
        responses.replace(
            ...
            json={
                ...
                'summary': {
                    ...
                    'rest': {
                        'duration': 0, 'candidates': 0, 'rate': 0}
                },
            },
            status=200,
        )
        ...
        result = self.cli(
            'subset', '--target', '10%', '--session', self.session, 'nunit', str(self.test_files_dir) + "/list.xml")
    ...
    def test_split_subset(self):
        ...
        result = self.cli(
            'split-subset', '--subset-id', 'subset/456', '--bin', '1/2', '--rest', rest.name, 'nunit')

        self.assertEqual(result.exit_code, 0)

        self.assertIn(
            'ParameterizedTests.MyTests.DivideTest(12,3)', result.output)
    ...
    def test_record_test_on_linux(self):
        result = self.cli(
            'record', 'tests',  '--session', self.session, 'nunit', str(self.test_files_dir) + "/output-linux.xml")
        self.assertEqual(result.exit_code, 0)
    ...
    def test_record_test_on_windows(self):
        result = self.cli(
            'record', 'tests',  '--session', self.session, 'nunit', str(self.test_files_dir) + "/output-windows.xml")
        self.assertEqual(result.exit_code, 0)
