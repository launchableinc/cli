...
class MinitestTest(CliTestCase):
    ...
    def test_record_test_minitest_chunked(self):
        result = self.cli(
            'record', 'tests',  '--session', self.session, '--post-chunk', 5, 'minitest', str(self.test_files_dir) + "/")
        self.assertEqual(result.exit_code, 0)
    ...
    def test_subset(self):
        ...
        result = self.cli(
            'subset', '--target', '20%', '--session', self.session, '--base', str(
                self.test_files_dir),
    ...
    def test_subset_split(self):
        ...
        result = self.cli(
            'subset', '--target', '20%', '--session', self.session, '--base', str(
                self.test_files_dir),
            '--split', 'minitest', str(self.test_files_dir) + "/test/**/*.rb")
    ...
    def test_split_subset(self):
        ...
        responses.replace(
            ...
            json={'testPaths': [[{'name': str(test_path)}]], 'rest': [
            ], 'subsettingId': 123},
            status=200,
        )
        ...
        result = self.cli(
            'split-subset', '--subset-id', 'subset/456', '--base', str(
                self.test_files_dir),
