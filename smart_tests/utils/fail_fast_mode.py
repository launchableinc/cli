...
class FailFastModeValidateParams:
    def __init__(self, command: Command | Tracking.Command, build: Optional[str] = None, is_no_build: bool = False,
...
def fail_fast_mode_validate(params: FailFastModeValidateParams):
    ...
    if params.command in (Command.RECORD_SESSION, Tracking.Command.RECORD_SESSION):
        _validate_record_session(params)
    if params.command in (Command.SUBSET, Tracking.Command.SUBSET):
        _validate_subset(params)
    if params.command in (Command.RECORD_TESTS, Tracking.Command.RECORD_TESTS):
        _validate_record_tests(params)


def _validate_require_session_option(params: FailFastModeValidateParams) -> List[str]:
    errors: List[str] = []
    # Get command name - handle both Command types
    if hasattr(params.command, 'display_name'):
        cmd_name = params.command.display_name()
    else:
        cmd_name = params.command.value.lower().replace('_', ' ')
    if params.session:
