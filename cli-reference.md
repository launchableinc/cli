...
| Option | Description | Required |
| :--- | :--- | :--- |
| `--build BUILD_NAME` | Name of the build being tested. \(See `record build --name`\) | Yes, but exclusive with `--session` |
| `--session SESSIONID` | ID of the test session \(see `record session`\) | Yes, but exclusive with `--name` |
